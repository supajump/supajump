...
add constraint check_provider check (provider in ('stripe'));
...
add constraint check_interval check (interval in ('day', 'week', 'month', 'year'));
...
--     );
create
or replace function public.get_organization_billing_status (lookup_org_id uuid) returns json as $$
DECLARE
    result RECORD;
BEGIN
    select s.id,
...
grant
execute on function public.get_organization_billing_status (uuid) to authenticated,
service_role;
